...
class ChaosAdminMixin:
    ...
    @override_settings(
        CHAOS={"mock_safe": True, "ignore_apps_request": ["admin"]},
    )
    def test_admin_list(self):
        self._call_mockfn(verb=models.verb_raise, probability=100)
        url = reverse(
            "admin:django_chaos_engineering_{}_changelist".format(self.modelstr),
        )
    ...
    def test_admin_change(self):
        ...
        url = reverse(
            "admin:django_chaos_engineering_{}_change".format(self.modelstr),
            args=(action.pk,),
        )
        self.assertEqual(200, self.c.get(url).status_code)
